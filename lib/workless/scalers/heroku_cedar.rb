  ...
  module Workless
    module Scaler
      class HerokuCedar < Base
        ...
        def self.up
          app_name = ENV['APP_NAME'] || ENV['HEROKU_APP_NAME']
          client.post_ps_scale(app_name, 'worker', self.workers_needed) if self.workers_needed > self.min_workers and self.workers < self.workers_needed
        end

        def self.down
          app_name = ENV['APP_NAME'] || ENV['HEROKU_APP_NAME']
          client.post_ps_scale(app_name, 'worker', self.min_workers) unless self.jobs.count > 0 or self.workers == self.min_workers
        end

        def self.workers
          app_name = ENV['APP_NAME'] || ENV['HEROKU_APP_NAME']
          client.get_ps(app_name).body.count { |p| p["process"] =~ /worker\.\d?/ }
        end
